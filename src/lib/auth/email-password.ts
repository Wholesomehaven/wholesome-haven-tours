...
export async function sendStaffResetPassword({
  ...
  rememberPreviewResetUrl(user.email, url);
  ...
  const html = `
    <div style="font-family:Georgia,serif;background:#fbfaf6;padding:24px;color:#3d3a32">
      ...
      <p style="margin:0 0 24px"><a href="${url}" style="background:#5a7344;color:#fff;padding:12px 18px;border-radius:8px;text-decoration:none">Choose a new password</a></p>
      <p style="margin:0 0 8px;font-size:13px">If the button does not work, paste this address into your browser:</p>
      <p style="margin:0 0 24px;font-size:13px;word-break:break-all">${url}</p>
